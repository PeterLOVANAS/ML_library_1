...
for e in range(epochs):
    ...
    with alive_bar(total = num_sample , title =f"Epoch {e+1}" , theme = "smooth" ) as bar:
        for x , y in zip(train_images , train_labels):
            ...
            output_gradient = binary_cross_entropy_prime(y, output)
...
while True:
    index = int(input("Enter a number (0-8999): "))
    if index != "stop":
        pass
    elif index == "stop":
        break
    img = test_images_1[1000:][index]
    lbl = test_labels_1[1000:][index]
    output = predict(modelV1 ,img)
    plt.imshow(img.reshape(28, 28), cmap="Greys")
    plt.title(f"Output layer answered : {np.argmax(output) } , label : {np.argmax(lbl)} ") # Index of output layer vector is according to the number
    plt.show()
